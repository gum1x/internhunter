"""NEOGOV / GovernmentJobs.com agency career pages (state & local government — complements the
federal USAJobs ingestor). Renders the agency board and reads schema.org JobPosting JSON-LD.
Best-effort; token is the agency slug from governmentjobs.com/careers/{agency}."""
...
@register_source
class NeogovSource(Source):
    ats: str = "neogov"
    tier: Tier = Tier.C
    needs_browser: bool = True

    def board_url(self, ref: BoardRef) -> str:
        return f"https://www.governmentjobs.com/careers/{ref.token}"
    ...
    async def fetch(self, ref: BoardRef, ctx: FetchContext) -> AsyncIterator[RawPosting]:
        ...
        try:
            ...
            ctx.logger.debug("neogov render failed for {}", ref.token)
            return
