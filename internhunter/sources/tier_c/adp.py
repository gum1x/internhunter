...
def _location(anchor: Any) -> str | None:
    ...
    if parent is None:
        return None
    node = parent.find(class_=lambda value: bool(value) and "location" in value)
    if node is None:
        return None
...
class AdpSource(Source):
    ...
    def board_url(self, ref: BoardRef) -> str:
        return f"https://jobs.adp.com/company/{ref.token}"

    async def fetch(self, ref: BoardRef, ctx: FetchContext) -> AsyncIterator[RawPosting]:
        if ctx.browser is None:
            ctx.logger.debug("adp poll skipped for {}: no browser", ref.token)
            return
        html = await ctx.browser.render(self.board_url(ref), wait_for="a")
