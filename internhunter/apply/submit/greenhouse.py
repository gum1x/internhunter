...
def parse_questions(payload: dict[str, Any]) -> list[FormField]:
    out: list[FormField] = []
...
class GreenhouseSubmitter(Submitter):
    ...
    def _job_url(self, job: Any) -> str:
        return (
            f"https://boards-api.greenhouse.io/v1/boards/{job.board_token}/jobs/{job.source_job_id}"
        )
