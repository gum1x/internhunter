...
def test_dashboard_basic_auth(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("INTERNHUNTER_AUTH_USER", "ryan")
    monkeypatch.setenv("INTERNHUNTER_AUTH_PASS", "s3cret")
    with TestClient(create_app()) as c:
        assert c.get("/").status_code == 401
        good = base64.b64encode(b"ryan:s3cret").decode()
        assert c.get("/", headers={"Authorization": f"Basic {good}"}).status_code == 200
        bad = base64.b64encode(b"ryan:nope").decode()
