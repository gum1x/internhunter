...
from internhunter.sources.tier_b.bamboohr import BambooHrSource

FIXTURE = Path(__file__).parent / "fixtures" / "bamboohr" / "jobs.json"
...
@pytest.mark.asyncio
async def test_bamboohr_poll_yields_normalized_jobs(fake_fetch_context: Any) -> None:
    source = BambooHrSource()
    ref = BoardRef(ats="bamboohr", token="acme", company="Acme")
    ...
    intern = jobs[0]
    assert intern.ats == "bamboohr"
    assert intern.board_token == "acme"
    assert intern.company == "Acme"
    assert intern.source_job_id == "4821"
    assert intern.title == "Software Engineering Intern"
    assert intern.canonical_url == "https://acme.bamboohr.com/careers/4821"
...
@pytest.mark.asyncio
async def test_bamboohr_poll_non_dict_response_yields_empty(fake_fetch_context: Any) -> None:
    from internhunter.sources.tier_b.bamboohr import BambooHrSource
